...
# Luckily this one is quick with Regex, just replace DCCCC, LXXXX, VIIII, XXXX, 
# and IIII with their two space counterparts, but for the purposes of measuring 
# length we can just replace with two spaces.
@timeit
def answer():
    f = open('p089_roman.txt', 'r')
    data = f.read()
    return len(data) - len(re.sub("DCCCC|LXXXX|VIIII|CCCC|XXXX|IIII", '  ', data))